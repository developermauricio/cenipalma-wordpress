...
const clickStar = (img, value) => {
  const src = img.src;
  const arrayNameImage = src.split("/");
  const nameImage = arrayNameImage[arrayNameImage.length - 1];
  const alt = document.querySelector(`a[href="${src}"] img`).getAttribute('alt');
  
  let dataImage = {
    nameImage: arrayNameImage[arrayNameImage.length - 1],
    qualification: value,
